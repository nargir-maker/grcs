...
function GenderPill({ gender, count, pct, color, dateLocale }: { gender: string; count: number; pct: number; color: string; dateLocale: string }) {
  ...
  return (
    <div className="flex-1 rounded-xl p-4 border" style={{ borderColor: `${color}33`, background: `${color}11` }}>
      ...
      <div className="font-bold text-xl" style={{ color }}>{count.toLocaleString(dateLocale)}</div>
      <div className="text-white/50 text-xs mt-0.5">{gender === 'F' ? t('women') : t('men')} · {pct.toFixed(1)}%</div>
...
function PioneerCard({ name, year, firstEvent, lastBrevetName, lastBrevetYear, gender }: {
  ...
  return (
    <div className="flex-1 rounded-xl border p-4" style={{ borderColor: `${color}33`, background: `${color}0A` }}>
      ...
      <div className="text-white/40 text-xs mt-1">{t('firstBrevetLabel', { year })}</div>
      {firstEvent && <div className="text-white/60 text-xs mt-0.5 truncate">{firstEvent}</div>}
      {lastBrevetYear && (
        <div className="text-white/30 text-xs mt-2">
          {t('lastBrevetLabel', { year: lastBrevetYear })}
...
export default function CommunityPage() {
  ...
  const t = useTranslations('community');
  const tb = useTranslations('brevets');
  const locale = useLocale();
  const dateLocale = locale === 'el' ? 'el-GR' : 'en-US';
  const months = tb.raw('months') as string[];
    ...
    return (
      <div className="min-h-screen bg-[#0A1628] flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-white/40 text-sm">{t('loadingStats')}</div>
        </div>
      </div>
    );
  }
  ...
  const chartTotal = t('chartTotal');
  const chartWomen = t('women');
  const chartNewMen = t('chartNewMen');
  const chartNewWomen = t('chartNewWomen');

  // Chart data: yearly active riders
  const yearlyData = data.sortedYears.map(y => ({
    year: y,
    [chartTotal]: data.totalByYear[y] ?? 0,
    [chartWomen]: data.womenByYear[y] ?? 0,
    [chartNewMen]: data.newMenByYear[y] ?? 0,
    [chartNewWomen]: data.newWomenByYear[y] ?? 0,
  }));

  // Seasonality data
  const monthData = Array.from({ length: 12 }, (_, i) => ({
    m: months[i],
    [chartTotal]: data.eventsByMonth[(i + 1).toString()] ?? 0,
    [chartWomen]: data.eventsByMonthWomen[(i + 1).toString()] ?? 0,
  }));
    ...
    <main className="min-h-screen bg-[#0A1628] pb-16">
      ...
      <div className="px-4 pt-8 pb-6 max-w-3xl mx-auto">
        ...
        <p className="text-white/40 text-sm mt-1">
          {t('subheading', { count: total.toLocaleString(dateLocale), years: data.sortedYears.length })}
        </p>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 space-y-5">
        ...
        {/* ── Gender split ─────────────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('genderSplitTitle')}</SectionTitle>
          <div className="flex gap-3">
            <GenderPill gender="F" count={data.totalWomen} pct={data.womenPct} color="#ec4899" dateLocale={dateLocale} />
            <GenderPill gender="M" count={data.totalMen}   pct={100 - data.womenPct} color="#60a5fa" dateLocale={dateLocale} />
          </div>
        </Card>

        {/* ── Distance distribution ─────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('distanceDistTitle')}</SectionTitle>
                ...
                return (
                  <div key={dist}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-white/40 text-xs">{t('ridersCount', { count: tot })} · {women}♀ {men}♂</span>
                    </div>
          ...
          <div className="flex gap-4 mt-3 text-xs text-white/30">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-pink-500 inline-block" />{t('women')}</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-500 inline-block" />{t('men')}</span>
          </div>
        </Card>

        {/* ── Yearly active riders ──────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('yearlyRidersTitle')}</SectionTitle>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={yearlyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey={chartTotal} stroke="#60a5fa" fill="rgba(96,165,250,0.15)" strokeWidth={2} dot={false} />
              <Area type="monotone" dataKey={chartWomen} stroke="#ec4899" fill="rgba(236,72,153,0.15)" strokeWidth={2} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
        ...
        {/* ── New riders per year ───────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('newRidersTitle')}</SectionTitle>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={yearlyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <YAxis tick={{ fill: 'rgba(255,255,255,0.3)', fontSize: 10 }} tickLine={false} />
              <Tooltip content={<ChartTooltip />} />
              <Bar dataKey={chartNewMen}   fill="#3b82f6" radius={[3, 3, 0, 0]} stackId="new" />
              <Bar dataKey={chartNewWomen} fill="#ec4899" radius={[3, 3, 0, 0]} stackId="new" />
            </BarChart>
          </ResponsiveContainer>
        </Card>
        ...
        {/* ── Pioneers ─────────────────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('pioneersTitle')}</SectionTitle>
          <div className="flex gap-3">
            <PioneerCard
        ...
        <Card>
          ...
          <div className="flex gap-2 mb-4">
            <div className="flex-1 rounded-xl p-3 text-center border border-pink-500/30 bg-pink-500/10">
              ...
              <div className="text-white/40 text-xs mt-0.5">{t('womenSrLabel')}</div>
            </div>
            <div className="flex-1 rounded-xl p-3 text-center border border-blue-500/30 bg-blue-500/10">
              ...
              <div className="text-white/40 text-xs mt-0.5">{t('menSrLabel')}</div>
            </div>
          </div>
        ...
        {/* ── Top Events ───────────────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('topEventsTitle')}</SectionTitle>
        ...
        {/* ── Seasonality ──────────────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('seasonalityTitle')}</SectionTitle>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={monthData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <YAxis tick={{ fill: 'rgba(255,255,255,0.3)', fontSize: 10 }} tickLine={false} />
              <Tooltip content={<ChartTooltip />} />
              <Bar dataKey={chartTotal} fill="rgba(96,165,250,0.6)"  radius={[3, 3, 0, 0]} />
              <Bar dataKey={chartWomen} fill="rgba(236,72,153,0.8)" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>
        ...
        {/* ── Streaks ──────────────────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('streaksTitle')}</SectionTitle>
          <div className="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-white/40 text-xs">{t('streakRecordSub', { years: data.globalMaxStreak })}</div>
            </div>
          </div>
            ...
            {streakEntries.slice(-8).reverse().map(({ years, count }) => {
              ...
              return (
                <div key={years} className="flex items-center gap-3">
                  <div className="text-white/50 text-xs w-14 text-right shrink-0">{years} {years === 1 ? t('yearSingular') : t('yearPlural')}</div>
        ...
        {/* ── Milestones ───────────────────────────────────────────── */}
        <Card>
          <SectionTitle>{t('milestonesTitle')}</SectionTitle>
