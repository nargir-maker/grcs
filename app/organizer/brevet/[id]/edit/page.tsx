...
// app/organizer/brevet/[id]/edit/page.tsx
// Edit an existing brevet — accessible to the owning organiser OR any admin user
...
function detectViaCities(trackPoints: { lat: number; lng: number }[]): string {
  ...
  for (let i = 1; i < trackPoints.length; i++) {
    const d = haversineKm(trackPoints[i-1].lat, trackPoints[i-1].lng, trackPoints[i].lat, trackPoints[i].lng);
    totalKm += d; distAcc += d;
    if (distAcc >= 2) { sampled.push({ ...trackPoints[i], idx: i, km: totalKm }); distAcc = 0; }
  }
  ...
  return hits
    ...
    .map(h => {
      ...
      return `${nameClean} (${Math.round(h.km)}χλμ)`;
    })
    .join(' - ');
}

interface GpxParsed {
  realKm: number; ascent: number; descent: number; wcs: number;
  startCoords: string; finishCoords: string;
...
function parseGpx(text: string): GpxParsed {
  ...
  const startPt = pts[0], finishPt = pts[pts.length - 1];
  ...
  const trackPoints = pts
    ...
    .map(pt => ({ lat: parseFloat(pt.getAttribute('lat') ?? '0'), lng: parseFloat(pt.getAttribute('lon') ?? '0') }))
    .filter(p => p.lat && p.lng);
  const km = Math.round(distKm * 10) / 10;
  return {
    realKm: km, ascent: Math.round(ascent), descent: Math.round(descent),
    wcs: km > 0 ? Math.round((ascent / km) * 100) / 100 : 0,
    startCoords: `${slat},${slng}`, finishCoords: `${flat},${flng}`,
    waypoints: wpts, trackPoints,
  };
}

// ── Types ─────────────────────────────────────────────────────────────────────
interface Control { km: number; name: string; isManned: boolean; lat: number; lng: number; }
...
interface FormState {
  ...
  gpxUrl: string; mapUrl: string;
  description: string; hasMedal: boolean; medalCost: string; entryCost: string; flecheData: string;
  controls: Control[];
}
...
export default function EditBrevetPage() {
  ...
  const [loadingBrevet, setLoadingBrevet] = useState(true);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await getDoc(doc(db, 'all_brevets', brevetId));
        ...
        const parseDateParts = (iso: string) => {
          if (!iso) return { date: '', time: '07:00' };
          const dt = new Date(iso.split('+')[0].split('Z')[0]);
          if (isNaN(dt.getTime())) return { date: '', time: '07:00' };
        ...
        setForm({
          ...
          certification:    info.certification  ?? 'A.C.P.',
        ...
        console.error('Load brevet error:', e);
        router.replace('/');
      } finally {
        setLoadingBrevet(false);
      }
    })();
  }, [brevetId]);

  // Admin check via NextAuth session
  useEffect(() => {
    if (sessionStatus === 'loading') return;
    if (!session?.user?.email) { setAdminChecked(true); return; }
    (async () => {
      try {
  ...
  // Authorization gate — once brevet + auth both settled
  useEffect(() => {
    if (loadingBrevet || !organizerLoaded || !adminChecked) return;
    if (isAdmin) { setAuthorized(true); return; }
    if (isOrganizer && organizer?.clubId === form.organizerId) { setAuthorized(true); return; }
    router.replace('/');
  }, [loadingBrevet, organizerLoaded, adminChecked, isAdmin, isOrganizer, organizer, form.organizerId]);
  ...
  async function handleGpxFile(file: File) {
    ...
    try {
      ...
      if (finishLat && finishLng) {
        try {
          const isSamePoint = Math.abs(startLat - finishLat) < 0.001 && Math.abs(startLng - finishLng) < 0.001;
          if (isSamePoint) {
  ...
  // ── Control helpers ───────────────────────────────────────────────────────────
  const addCtrl    = () => set('controls', [...form.controls, { km:0, name:'', isManned:false, lat:0, lng:0 }]);
  const removeCtrl = (i: number) => set('controls', form.controls.filter((_, j) => j !== i));
  const updateCtrl = (i: number, key: keyof Control, val: any) =>
    set('controls', form.controls.map((c, j) => j===i ? {...c,[key]:val} : c));

  // ── Save (updateDoc — keeps createdAt unchanged) ──────────────────────────────
  async function handleSave() {
    ...
    try {
      ...
      await updateDoc(doc(db, 'all_brevets', brevetId), {
        info: {
      ...
      setTimeout(() => {
        if (isAdmin && !isOrganizer) router.push('/admin');
  ...
  // ── Render guards ─────────────────────────────────────────────────────────────
  const settling = loadingBrevet || !organizerLoaded || !adminChecked;
  if (settling || !authorized) {
    return (
  ...
  const endDt  = endDatetime();
  const backHref = isAdmin && !isOrganizer ? '/admin' : '/organizer/dashboard';
    ...
    <div className="min-h-screen bg-[#0A1628] px-4 py-10">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          <Link href={backHref} className="text-white/40 hover:text-white text-sm transition-colors">
            ← {isAdmin && !isOrganizer ? 'Admin' : 'Dashboard'}
          </Link>
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-white">Επεξεργασία Brevet</h1>
            <p className="text-white/30 text-xs mt-0.5 font-mono">{brevetId}</p>
          </div>
          {isAdmin && (
            <span className="text-xs bg-amber-500/20 border border-amber-400/30
              text-amber-300 px-2.5 py-1 rounded-full font-semibold">
              👑 Admin
            </span>
          )}
        </div>
        ...
        <Section title="📋 Βασικά στοιχεία">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Πιστοποίηση">
              <select className={sel} value={form.certification} onChange={e => set('certification', e.target.value)}>
                <option>A.C.P.</option>
                <option>H.A.R.</option>
              </select>
            </Field>
          ...
          <Field label="Μέγιστη διάρκεια (ώρες)" hint="προσυμπληρώνεται από απόσταση">
            ...
            {endDt && (
              <p className="text-white/40 text-xs mt-1.5">
                Λήξη: {endDt.toLocaleDateString('el-GR', { weekday:'short', day:'numeric', month:'short' })}{' '}
        ...
        <Section title="🔘 Κατάσταση">
          <div className="space-y-4">
            ...
            <Field label="Σύνδεσμος εξωτερικής εγγραφής (URL)"
              hint="αν έχει, το κουμπί Εγγραφή θα ανοίγει αυτόν τον σύνδεσμο">
              <input
                type="url"
                placeholder="https://..."
        ...
        <Section title="🗺️ Διαδρομή & GPX">
          <Field label="Τοπικό GPX αρχείο" hint="δεν ανεβαίνει — αναλύεται τοπικά για αυτόματη ανανέωση στοιχείων">
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => gpxRef.current?.click()}
                ...
                📁 {gpxParsed ? 'Αντικατάσταση GPX' : 'Επιλογή GPX'}
              </button>
              {gpxParsed && <span className="text-green-400 text-sm">✓ Αναλύθηκε</span>}
              {!gpxParsed && form.gpxUrl && <span className="text-white/30 text-xs">Υπάρχον GPX URL παραμένει</span>}
            </div>
          ...
          {gpxTrackPoints.length > 0 && (
            <div className="mb-4">
              <p className="text-white/40 text-xs font-semibold uppercase tracking-wider mb-2">
                ...
                <span className="ml-2 text-white/20 normal-case font-normal">{gpxTrackPoints.length} σημεία</span>
              </p>
              <GpxPreviewMap
          ...
          <Field label="GPX URL" hint="GitHub raw link">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Πόλη εκκίνησης" hint={geocoding ? '🔍 Αναζήτηση...' : 'από GPX ή χειροκίνητα'}>
              <input className={inp} value={form.start}
                onChange={e => set('start', e.target.value)}
                placeholder={geocoding ? 'Αναζήτηση τοποθεσίας...' : 'π.χ. ΚΑΛΠΑΚΙ'} />
            </Field>
            <Field label="Πόλη τερματισμού" hint={geocoding ? '🔍 Αναζήτηση...' : 'από GPX ή χειροκίνητα'}>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Field label="WCS" hint="ανηφόρα/km">
              <input type="number" step="0.01" className={inp} value={form.wcs}
        ...
        <Section title="📍 Σημεία Ελέγχου">
          {form.controls.length === 0
            ? <p className="text-white/30 text-sm mb-3">Δεν υπάρχουν σημεία ελέγχου.</p>
            : (
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="relative shrink-0">
                        ...
                        <span className="absolute right-3 top-1/2 -translate-y-1/2
                          text-white/30 text-xs font-medium pointer-events-none">km</span>
                      </div>
        ...
        {saved && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-xl px-4 py-3
            text-green-400 text-sm mb-4">✓ Αποθηκεύτηκε! Μεταφορά...</div>
        )}
